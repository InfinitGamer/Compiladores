...
antlrcpp::Any TypeCheckVisitor::visitForStmt(AslParser::ForStmtContext *ctx){
  DEBUG_ENTER();
  visit(ctx->ident());
  TypesMgr::TypeId vt = getTypeDecor(ctx->ident());
  if(not Types.isErrorTy(vt) and not Types.isIntegerTy(vt)){
    Errors.forRequireIntegerVar(ctx->ident());
  }
  for(auto& var : ctx->expr()){
      visit(var);
      TypesMgr::TypeId t = getTypeDecor(var);
      if(not Types.isErrorTy(t) and not Types.isIntegerTy(t)){
        Errors.forRequireIntegerExpr(var); 
      }
  }
   
  if(ctx->expr().size() >3) Errors.numberOfRangeExpressions(ctx);
  visit(ctx->statements());
  DEBUG_EXIT();
  return 0;
}
antlrcpp::Any TypeCheckVisitor::visitMaxFunction(AslParser::MaxFunctionContext *ctx){
  DEBUG_ENTER();
  TypesMgr::TypeId tipo = Types.createErrorTy();
  if(ctx->expr().size() < 2) Errors.numberOfMaxArguments(ctx);
  bool isFloat = false;
  ...
  for(auto& var :  ctx->expr()){
    visit(var);
    TypesMgr::TypeId t = getTypeDecor(var);
    if(Types.isCharacterTy(t)) isCharacter = true;
    else if(Types.isFloatTy(t)) isFloat = true;
    else if(Types.isIntegerTy(t)) isInteger = true;
    else if(not Types.isErrorTy(t)) isDiffTy = true;
  }
  if(isDiffTy or (isCharacter and (isInteger or isFloat))){
    Errors.incompatibleMaxArguments(ctx);
  } 
  else{
    if(isCharacter) tipo = Types.createCharacterTy();
    else if(isFloat) tipo = Types.createFloatTy();
    else if(isInteger) tipo = Types.createIntegerTy();
  }
  putTypeDecor(ctx, tipo);
  DEBUG_EXIT();
  return 0;
}
