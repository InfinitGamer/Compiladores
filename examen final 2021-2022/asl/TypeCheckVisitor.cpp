...
antlrcpp::Any TypeCheckVisitor::visitFactorial(AslParser::FactorialContext *ctx){
  DEBUG_ENTER();
  visit(ctx->expr());
  TypesMgr::TypeId ret = getTypeDecor(ctx->expr());
  if(not Types.isErrorTy(ret) and not Types.isIntegerTy(ret)){
...
antlrcpp::Any TypeCheckVisitor::visitMatrixIndex(AslParser::MatrixIndexContext *ctx){
  DEBUG_ENTER();
  visit(ctx->left_expr());
  TypesMgr::TypeId t1 = getTypeDecor(ctx->left_expr());
  ...
  bool matrix_check = Types.isMatrixTy(t1);
  if(not Types.isErrorTy(t1) and not matrix_check){
  ...
  if(matrix_check) t1 = Types.getMatrixElemType(t1);
  putTypeDecor(ctx, t1);
  putIsLValueDecor(ctx, b);
  DEBUG_EXIT();
  return 0;
