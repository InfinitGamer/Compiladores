...
antlrcpp::Any CodeGenVisitor::visitFactorial(AslParser::FactorialContext *ctx){
  ...
  std::string temp ='%'+ codeCounters.newTEMP();
  code1 = code1 ||instruction::LT(temp, addr1, zero)
  ||instruction::FJUMP(temp, labelEndIf) ||instruction::HALT(code::INVALID_INTEGER_OPERAND)|| instruction::LABEL(labelEndIf);

  std::string ret ='%'+ codeCounters.newTEMP();
  code1 = code1 || instruction::ILOAD(ret, "1");
  ...
  std::string labelWhileBegin = "while"+label;
  std::string labelWhileEnd = "endwhile"+label;
  code1 = code1 || instruction::LABEL(labelWhileBegin);
  std::string comp = '%'+ codeCounters.newTEMP();
  code1 = code1 ||instruction::LT(comp, zero, var);
  code1 = code1 ||instruction::FJUMP(comp, labelWhileEnd);
  code1 = code1 ||instruction::MUL(ret,ret,var);
  code1 = code1 ||instruction::SUB(var,var, one);
  code1 = code1 ||instruction::UJUMP(labelWhileBegin);
  code1 = code1 ||instruction::LABEL(labelWhileEnd);
  codAt1.addr = ret;
  DEBUG_EXIT();
  return codAt1;
}
antlrcpp::Any CodeGenVisitor::visitFunction(AslParser::FunctionContext *ctx) {
  ...
  for (int i = 0; i < (int)ctx->param().size(); i++){
    ...
    std::string type = Types.to_string(getBasicType(tParam));
...
antlrcpp::Any CodeGenVisitor::visitVariable_decl(AslParser::Variable_declContext *ctx) {
  ...
  for(auto& varID : ctx->ID()){
    lvars.push_back(var{varID->getText(), Types.to_string(getBasicType(t1)), size});
  }
  DEBUG_EXIT();
  return lvars;
}
...
antlrcpp::Any CodeGenVisitor::visitAssignStmt(AslParser::AssignStmtContext *ctx) {
  ...
  else if(Types.isMatrixTy(tid1) && Types.isMatrixTy(tid2)){
    std::string temp1 = addr1;
    std::string temp2 = addr2;
    if(Symbols.isParameterClass(temp1)){
        temp1 = '%'+codeCounters.newTEMP();
...
antlrcpp::Any CodeGenVisitor::visitMatrixIndex(AslParser::MatrixIndexContext *ctx){
  DEBUG_ENTER();
  CodeAttribs codAts = visit(ctx->left_expr());
    std::string         addr1 = codAts.addr;
    instructionList  &  code1 = codAts.code;
    ...
    std::string         addr3 = codAts2.addr;
    instructionList     code3 = codAts2.code;
    TypesMgr::TypeId t = getTypeDecor(ctx->left_expr());

    int size = Types.getMatrixCols(t);
    std::string temp1 =  '%' + codeCounters.newTEMP(); 
    code1 = code1 || code2 ||code3 || instruction::ILOAD(temp1, std::to_string(size));
    std::string temp2 =  '%' + codeCounters.newTEMP();
    code1 = code1 || instruction::MUL(temp2, addr2, temp1);
    std::string temp3 =  '%' + codeCounters.newTEMP();
    code1 = code1 ||instruction::ADD(temp2,temp2,addr3);
    std::string temp5 = addr1;
    int size2 = Types.getMatrixSize(t);
    std::string label = codeCounters.newLabelIF();
    std::string labelEndIf = "endif"+label;
    std::string temp6 =  '%' + codeCounters.newTEMP();
    std::string temp7 =  '%' + codeCounters.newTEMP();
    code1 = code1 || instruction::ILOAD(temp6,std::to_string(size2));
    code1 = code1 || instruction::LE(temp7, temp6, temp2);
    code1 = code1 || instruction::FJUMP(temp7,labelEndIf);
    code1 = code1 || instruction::HALT(code::INDEX_OUT_OF_RANGE);
    code1 = code1 || instruction::LABEL(labelEndIf);
    ...
    std::string temp4 =  '%' + codeCounters.newTEMP();
	  code1 = code1 || instruction::LOADX(temp4, temp5, temp2);
    codAts.addr = temp4;
	  codAts.offs = "";
  DEBUG_EXIT();
  return codAts;
...
int CodeGenVisitor::getSize(TypesMgr::TypeId t){
  if(Types.isPrimitiveTy(t)) return 1;
    ...
    if(Types.isArrayTy(t)){
      ...
      int i = Types.getMatrixSize(t);
      TypesMgr::TypeId t_child = Types.getMatrixElemType(t);
      return i * getSize(t_child);
    }
  }
}
