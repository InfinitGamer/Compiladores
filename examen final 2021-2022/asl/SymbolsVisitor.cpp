...
antlrcpp::Any SymbolsVisitor::visitMatrixType(AslParser::MatrixTypeContext *ctx){
  DEBUG_ENTER();
  std::string tempStr0 = ctx->INTVAL(0)->getText();
  std::string tempStr1 = ctx->INTVAL(1)->getText();
  int size0 = stoi(tempStr0);
  int size1 = stoi(tempStr1);
  visit(ctx->t);
  TypesMgr::TypeId t1 = getTypeDecor(ctx->t);
  TypesMgr::TypeId t = Types.createErrorTy();
  if (not Types.isErrorTy(t1))
  {
    t = Types.createMatrixTy(size0,size1, t1);
  }
  putTypeDecor(ctx, t);
  DEBUG_EXIT();
  return 0;
