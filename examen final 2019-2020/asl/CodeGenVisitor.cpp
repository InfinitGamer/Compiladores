...
antlrcpp::Any CodeGenVisitor::visitFilter(AslParser::FilterContext *ctx){
  ...
  c = c || instruction::ILOAD(result, "0");
...
antlrcpp::Any CodeGenVisitor::visitSum(AslParser::SumContext *ctx){
  ...
  std::string & addr = codeAts.addr;
  instructionList & code = codeAts.code;
  TypesMgr::TypeId tret = getTypeDecor(ctx);
  addr = '%' + codeCounters.newTEMP();
  if(Types.isFloatTy(tret)){
    code = code || instruction::FLOAD(addr,"0.0");
  }
  else{
    code = code || instruction::ILOAD(addr,"0");
  }
  for(auto & var : ctx -> expr()){
    CodeAttribs     && codAtsE1 = visit(var);
    std::string           addr1 = codAtsE1.addr;
    // std::string           offs1 = codAtsE1.offs;
    instructionList &     code1 = codAtsE1.code;
    ...
    code = code || code1;
    std::string temp = addr1;
    if(Types.isIntegerTy(tvar) and Types.isFloatTy(tret)){
      temp = '%' + codeCounters.newTEMP();
      code = code || instruction::FLOAT(temp, addr1);
    }
    if(Types.isFloatTy(tret)){
      code = code || instruction::FADD(addr,addr,temp);
    } 
    else{
      code = code || instruction::ADD(addr,addr,temp);;
    }
  }
  DEBUG_EXIT();
  return codeAts;
