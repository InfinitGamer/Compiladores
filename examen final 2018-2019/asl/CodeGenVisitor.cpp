...
antlrcpp::Any CodeGenVisitor::visitArrayIndex(AslParser::ArrayIndexContext *ctx){
  DEBUG_ENTER();
    ...
    code1 = code1 || instruction::FJUMP(cond3, labelEndIf) || throwRuntimeErrorTCode(ctx)
...
antlrcpp::Any CodeGenVisitor::visitMax(AslParser::MaxContext *ctx){
  ...
  TypesMgr::TypeId tbasic = getBasicType(t);
  unsigned int size = Types.getArraySize(t);
  std::string direccion = addr1;
  if(Symbols.isParameterClass(direccion)){
    direccion = '%' + codeCounters.newTEMP();
    code1 = code1 || instruction::LOAD(direccion,addr1);
  }
  std::string index = '%' + codeCounters.newTEMP();
  std::string lim = '%' + codeCounters.newTEMP();
  std::string one = '%' + codeCounters.newTEMP();
  code1 = code1 || instruction::ILOAD(index, "0");
  code1 = code1 || instruction::ILOAD(lim, std::to_string(size));
  code1 = code1 || instruction::ILOAD(one, "1");
  std::string res = '%' + codeCounters.newTEMP();
  code1 = code1 || instruction::LOADX(res, direccion, index);
  code1 = code1 || instruction::ADD(index,index, one);
  ...
  std::string temp1 = '%' + codeCounters.newTEMP();
  code1 = code1 || instruction::LT(temp1, index, lim);
  code1 = code1 || instruction::FJUMP(temp1, labelEnd);

  std::string temp2 = '%' + codeCounters.newTEMP();
  code1 = code1 || instruction::LOADX(temp2, direccion, index);

  std::string label1 = codeCounters.newLabelIF();
  std::string labelEndIf = "endif"+label1;

  std::string temp3 = '%' + codeCounters.newTEMP();
  if(Types.isFloatTy(tbasic)){
    code1 = code1 || instruction::FLT(temp3, res, temp2);
  }
  else{
    code1 = code1 || instruction::LT(temp3, res, temp2);
  }
  code1 = code1 || instruction::FJUMP(temp3, labelEndIf);
  code1 = code1 || instruction::LOAD(res, temp2);
