...
antlrcpp::Any TypeCheckVisitor::visitPack(AslParser::PackContext *ctx){
  ...
  TypesMgr::TypeId vector_ty = getTypeDecor(ctx->left_expr());
  if(not Types.isErrorTy(vector_ty) and not Types.isArrayTy(vector_ty)){
    Errors.packUnpackWithNonArray(ctx);
  }
  //no queremos que si hay error entre en el else por lo tanto else if
  else if(Types.isArrayTy(vector_ty)){
    unsigned int size = ctx->expr().size();
    unsigned int vector_size = Types.getArraySize(vector_ty);
    if(size != vector_size){
      Errors.packUnpackNumberOfItemsMismatch(ctx);
    }
    TypesMgr::TypeId  elemTy= Types.getArrayElemType(vector_ty);
    for(int i = 0; i < size; i++){
       TypesMgr::TypeId exprTy = getTypeDecor(ctx->expr(i));
       if(not Types.copyableTypes(elemTy,exprTy)){
...
antlrcpp::Any TypeCheckVisitor::visitUnpack(AslParser::UnpackContext *ctx){
  ...
  //hacemos visitas
  visit(ctx->vector);
  for(auto& exp : ctx->left_expr()){
    visit(exp);
  }
  TypesMgr::TypeId vector_ty = getTypeDecor(ctx->vector);
  if(not Types.isErrorTy(vector_ty) and not Types.isArrayTy(vector_ty)){
    Errors.packUnpackWithNonArray(ctx);
  }
  //no queremos que si hay error entre en el else por lo tanto else if
  else if(Types.isArrayTy(vector_ty)){
    unsigned int size = ctx->left_expr().size()-1;
    unsigned int vector_size = Types.getArraySize(vector_ty);
    if(size != vector_size){
      Errors.packUnpackNumberOfItemsMismatch(ctx);
    }
    ...
    for(int i = 1; i < size; i++){
       TypesMgr::TypeId exprTy = getTypeDecor(ctx->left_expr(i));
       if(not Types.copyableTypes(exprTy,elemTy)){
        Errors.packUnpackIncompatibleTypes(ctx, i);
       }
    }
  }
  DEBUG_EXIT();
...
antlrcpp::Any TypeCheckVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  if(ctx->INTVAL()){
    ...
    else if(Types.isTupleTy(t1)){
      int size = Types.getTupleSize(t1);
      int value = stoi(ctx->INTVAL()->getText());
      if(size <= value)Errors.nonExistentFieldInTuple(ctx),t1 = Types.createErrorTy();
      else{
        t1 =Types.getTupleFieldType(t1, value);
...
antlrcpp::Any TypeCheckVisitor::visitTuplaAccess(AslParser::TuplaAccessContext *ctx){
  DEBUG_ENTER();
  visit(ctx->ident());
  TypesMgr::TypeId elem = Types.createErrorTy();
  TypesMgr::TypeId type = getTypeDecor(ctx->ident());
  if(not Types.isErrorTy(type) and not Types.isTupleTy(type)){
    Errors.nonTupleInTupleAccess(ctx);
  }
  else if(Types.isTupleTy(type)){
    int size = Types.getTupleSize(type);
    int value = stoi(ctx->INTVAL()->getText());
    if(size <= value)Errors.nonExistentFieldInTuple(ctx);
    else{
       elem =Types.getTupleFieldType(type, value);
