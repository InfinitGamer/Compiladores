...
antlrcpp::Any SymbolsVisitor::visitTupla(AslParser::TuplaContext *ctx){
 DEBUG_ENTER();
 std::vector<TypesMgr::TypeId> v;
 for(auto& type : ctx->BASIC_TYPE()){
  if(type->getText() == "int") v.push_back(Types.createIntegerTy());
  else if(type->getText() == "float")v.push_back(Types.createFloatTy());
  else if(type->getText() == "bool")v.push_back(Types.createBooleanTy());
  else if(type->getText() == "char")v.push_back(Types.createCharacterTy());
 }
 TypesMgr::TypeId type =Types.createTupleTy(v);
